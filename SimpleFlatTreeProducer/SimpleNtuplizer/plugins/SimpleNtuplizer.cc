...
SimpleNtuplizer::SimpleNtuplizer(const edm::ParameterSet& iConfig):
    ...
    electronTree_->Branch("SF_ID", &SF_ID_e);
    electronTree_->Branch("tp_tagSF_RECO", &tp_tagSF_RECO);
    electronTree_->Branch("tp_tagSF_ID", &tp_tagSF_ID);
...
void SimpleNtuplizer::analyze( const edm::Event& iEvent, const edm::EventSetup& iSetup ){
	...
	const edm::Ptr<pat::Electron> elPtr(electrons, ele - electrons->begin() );
	bool passID = (*tight_id_decisions)[elPtr];
	if (passID) passID_e = 1; else passID_e = 0;
	ele++;
